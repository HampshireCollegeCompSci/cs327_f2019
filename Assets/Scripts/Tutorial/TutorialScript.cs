...
public class TutorialScript : MonoBehaviour
{
    ...
    private void LoadSave(List<string> command)
    {
        if (command.Count != 2)
        {
            Debug.LogError($"command: \"{command}\" does not contain exactly 2 entries");
            return;
        }
    ...
    private void HighlightObject(List<string> command)
    {
        ...
        {
            Debug.LogError($"command: \"{command}\" {e.Message}");
        }
    }
    ...
    private void HighlightToken(List<string> command)
    {
        ...
        try
        {
            ...
            if (command[1] == "Reactor")
            {
                ...
                {
                    throw new FormatException($"contains an invalid token index. there are only {reactorCardList.Count} token(s) to choose from in reactor {objectIndex}");
                }
            ...
            else if (command[1] == "Foundation")
            {
                ...
                {
                    throw new FormatException($"contains an invalid token index. there are only {foundationCardList.Count} token(s) to choose from in foundation {objectIndex}");
                }
            ...
            else if (command[1] == "WastePile")
            {
                if (objectIndex != 0)
                {
                    throw new FormatException("contains an invalid waste pile index for the 2nd command, must be 0");
                }
                ...
                {
                    throw new FormatException($"contains an invalid token index. there are only {wastePileCardList.Count} token(s) to choose from in the waste pile");
                }
